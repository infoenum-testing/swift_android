...
import com.swift.dating.common.CustomPagerAdapter
import com.swift.dating.data.network.CallServer
import com.swift.dating.model.DetailTagModel
...
class CardProfileAdapter(
        var context: Context,
        private var spots: List<User> = emptyList(),
        var height: Int,
        var listener: UserCardActivity
) : RecyclerView.Adapter<CardProfileAdapter.ViewHolder>() {
...
//        }
        ...
        val layoutParams = holder.name.layoutParams as ConstraintLayout.LayoutParams
        layoutParams.topMargin = 30.toDp(context)
        layoutParams.marginStart = 0.toDp(context)
        layoutParams.marginEnd = 0.toDp(context)
        layoutParams.bottomMargin = 0.toDp(context)
        //holder.name.setLayoutParams(params)
        holder.name.layoutParams=layoutParams

        /*val newLayoutParams: ConstraintLayout.LayoutParams = toolbar.getLayoutParams() as ConstraintLayout.LayoutParams
...
*/
        ...
        if (spot.profileOfUser.distanceFromMatch!=null&&!TextUtils.isEmpty(spot.profileOfUser.distanceFromMatch.toString())) {
            holder.distance.text = spot.profileOfUser.distanceFromMatch.toString().split(".")[0] + " miles away"
        }
        ...
        Log.e("TAG", "setData: "+spot.profileOfUser.height)
...
public fun Int.toDp(context: Context):Int = TypedValue.applyDimension(
