...
public class RangeSeekBar<T extends Number> extends AppCompatImageView {
    ...
    public static final int ACTIVE_COLOR = Color.parseColor("#42536C");
    public static final int INVALID_POINTER_ID = 255;
    public static final int ACTION_POINTER_INDEX_MASK = 0x0000ff00, ACTION_POINTER_INDEX_SHIFT = 8;
    public static Integer DEFAULT_MINIMUM = 18;
    public static Integer DEFAULT_MAXIMUM = 80;
    public static final int HEIGHT_IN_DP = 30;
    public static final int TEXT_LATERAL_PADDING_IN_DP = 3;
    private static final int INITIAL_PADDING_IN_DP = 8;
    private static final int DEFAULT_TEXT_SIZE_IN_DP = 1;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.ic_slide;
        int thumbPressed = R.drawable.ic_slide;
        int thumbDisabled = R.drawable.ic_slide;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = CommonUtils.dpToPx(context, 2);
        int defaultShadowXOffset = CommonUtils.dpToPx(context, 0);
        int defaultShadowBlur = CommonUtils.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            mInternalPad = CommonUtils.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = CommonUtils.dpToPx(context, LINE_HEIGHT_IN_DP);
            mActiveColor = ACTIVE_COLOR;
            mDefaultColor = Color.parseColor("#E8E8E8");
            mAlwaysActive = false;
            mShowTextAboveThumbs = true;
            mTextAboveThumbsColor = Color.parseColor("#4e576c");
            ...
            try {
                ...
                if (normalDrawable != null) {
                    thumbImage = CommonUtils.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = CommonUtils.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = CommonUtils.drawableToBitmap(pressedDrawable);
                }
        ...
        mTextSize = CommonUtils.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        mDistanceToTop = CommonUtils.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        mTextOffset = !mShowTextAboveThumbs ? 0 : this.mTextSize + CommonUtils.dpToPx(context,
                DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP) + this.mDistanceToTop;

        mRect = new RectF(padding, mTextOffset + mThumbHalfHeight - barHeight / 2,
                getWidth() - padding, mTextOffset + mThumbHalfHeight + barHeight / 2);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!mShowTextAboveThumbs ? 0 : CommonUtils.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (mShowTextAboveThumbs && !selectedValuesAreDefault) {
            ...
            int offset = CommonUtils.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
            ...
            Log.e("normalizedToScreen", normalizedToScreen(normalizedMaxValue) - maxTextWidth * 0.5f + "--");
    ...
    /**
     * enumeration used to convert between Numbers and doubles.
     *
    ...
    /**
     * Callback listener interfaces to notify about changed range values.
     *
